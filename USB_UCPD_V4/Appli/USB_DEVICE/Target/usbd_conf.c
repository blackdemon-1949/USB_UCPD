...
/* PHY clock / VDD33USB readiness, as observed by the *last* HAL_PCD_Init()
   attempt.  HAL_PCD_MspInit() clears it at entry before re-evaluating the
   two RCC muxes and the PWR detector, so a retry really re-tests the
   hardware instead of inheriting a stale failure from the first boot. */
...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  {
    /* Fresh attempt: assume OK until this attempt proves otherwise. */
    s_usb_clock_ok = 1U;
...
/**
  ...
  * @retval USBD status
  */
/* Read-only view of the PHY/VDD33USB gate, for the status line and for the
   deferred bring-up in usb_device.c. */
uint8_t USBD_LL_ClockOk(void)
{
