...
/*---------- -----------*/
/* LPM stays OFF on purpose: the PCD runs with lpm_enable DISABLED and the
 * CDC class never serves a BOS descriptor (GetBOSDescriptor is NULL), so an
 * advertised LPM capability would be a contract lie - a host attempting L1
 * would exercise the suspend path without a validated resume/TxState story.
 * Compiling the dead BOS/LPM branches out changes nothing on the wire. */
