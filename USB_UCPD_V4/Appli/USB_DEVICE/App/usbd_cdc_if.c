...
/**
  * @brief  Resynchronise the CDC TX state after bus suspend / reset races.
  * @note   Clears the class TxState flag under IRQ mask.  No FIFO touch:
  *         safe to call from thread context (APP_LOG watchdog/suspend) while
  *         the bus is idle; the next CDC_Transmit_HS reprograms the endpoint
  *         from scratch (SetTxBuffer + TransmitPacket).  Never call from ISR.
  */
void CDC_ResyncTx_HS(void)
{
  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef *)hUsbDeviceHS.pClassData;
  uint32_t pm;
