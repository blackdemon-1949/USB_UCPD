...
{
  {
    ...
    .VDM_ProductTypeUFPorCP     = PRODUCT_TYPE_ACTIVE_CABLE, /*!< Product type UFP or CablePlug of the UUT based on @ref USBPD_ProductType_TypeDef */
  }
};
...
/* USER CODE BEGIN USBPD_VDM_InformIdentity */
  (void)SOPType;
  if (CommandStatus == SVDM_RESPONDER_ACK)
  {
    APP_PD_PrintIdentity(pIdentity, 1U);
    if (pIdentity != NULL)
    {
      APIE_Profile_OnIdentity(pIdentity->IDHeader.b20.VID,
                              pIdentity->ProductVDO.b.USBProductId, 0U, 0U);
    }
    APIE_OnVdmIdentity(PortNum, 1U);
  }
  else
  {
    APP_PD_PrintIdentity(NULL, 0U);
    APIE_OnVdmIdentity(PortNum, 0U);
  }
...
/* USER CODE BEGIN USBPD_VDM_InformSVID */
  ...
  if (CommandStatus == SVDM_RESPONDER_ACK)
  {
    APP_PD_PrintSvids(pListSVID, 1U);
    APIE_OnVdmSvids(PortNum, (pListSVID != NULL) ? pListSVID->SVIDs : NULL,
                    (pListSVID != NULL) ? pListSVID->NumSVIDs : 0U, 1U);
  }
  else
  {
    APP_PD_PrintSvids(NULL, 0U);
    APIE_OnVdmSvids(PortNum, NULL, 0U, 0U);
  }
...
/* USER CODE BEGIN USBPD_VDM_InformMode */
  ...
  if (CommandStatus == SVDM_RESPONDER_ACK)
  {
    APP_PD_PrintModes(pModesInfo, 1U);
  }
  else
  {
    APP_PD_PrintModes(NULL, 0U);
  }
