...
/* USER CODE BEGIN USBPD_DPM_Notification */
  APP_PD_OnNotify(PortNum, EventVal);
  ...
  switch (DataId)
  {
    ...
    case USBPD_CORE_SNK_EXTENDED_CAPA:
      *Size = sizeof(USBPD_SKEDB_TypeDef);
      (void)memcpy(Ptr, (uint8_t *)&DPM_USER_Settings[PortNum].DPM_SNKExtendedCapa, *Size);
      break;
#endif
    default:
      *Size = 0U;
...
/* USER CODE BEGIN USBPD_DPM_SetDataInfo */
  switch (DataId)
  {
    case USBPD_CORE_DATATYPE_RCV_SRC_PDO:
      APP_PD_StoreSrcPDO(PortNum, Ptr, Size);
      break;
    case USBPD_CORE_DATATYPE_RDO_POSITION:
      if (Size == 4U)
      {
        uint32_t pos = 0U;
        (void)memcpy((uint8_t *)&pos, Ptr, 4U);
        APP_PD_Port[PortNum].RDOPosition = pos;
      }
      break;
    /* Source-detail replies, decoded into plain English by APP_PD_OnDataInfo */
    ...
    case USBPD_CORE_BATTERY_STATUS:
      APP_PD_OnDataInfo(PortNum, DataId, Ptr, Size);
      break;
    default:
      break;
  }
...
/* USER CODE BEGIN USBPD_DPM_SNK_EvaluateCapabilities */
  APP_PD_Evaluate(PortNum, PtrRequestData, PtrPowerObjectType);
...
/* USER CODE BEGIN USBPD_DPM_ExtendedMessageReceived */
  APP_PD_OnExtendedMessage(PortNum, MsgType, ptrData, DataSize);
...
/* USER CODE BEGIN USBPD_DPM_RequestMessageRequest */
  _status = APP_PD_SendRequest(PortNum, IndexSrcPDO, RequestedVoltage, 0U);
