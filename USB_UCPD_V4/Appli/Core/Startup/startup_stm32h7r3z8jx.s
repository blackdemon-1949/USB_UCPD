...
/* Make sure the ITCM is enabled before the RAM functions are copied into it.
   The TCMs are enabled out of reset, but the bootloader runs first - if it
   ever turned ITCM off, the copy below would silently do nothing and the NOR
   driver would execute whatever stale data the TCM held. */
  ldr r0, =0xE000EF50            /* SCB->ITCMCR */
  ldr r1, [r0]
  orr r1, r1, #1                 /* EN */
  str r1, [r0]
  dsb
...
/* Copy the .ramfunc section (XSPI RAM-mode driver) from its load address in
   the flash image to ITCM.  It has to be resident before ANY NOR access,
   because programming the external flash switches the XiP window off. */
  ldr r2, =_siramfunc
  ldr r0, =_sramfunc
  ldr r1, =_eramfunc
