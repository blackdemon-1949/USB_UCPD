...
void APP_CLI_OnRx(const uint8_t *data, uint32_t len)
{
  ...
  for (i = 0; i < len; i++)
  {
    ...
    {
      /* Ring full between super-loop drains (paste burst > 256 B): count
       * the loss instead of failing silently, mirroring EXT_UART_RxDropped.
       * Only ever incremented here, under IRQ mask. */
      s_rx_dropped++;
    }
...
void APP_CLI_PrintHelp(void)
{
  APP_LOG_Write(
    ...
    "  oled page <0-5>        0 volt  1 current  2 power  3 requested\r\n"
    "                        4 protocol  5 SoC die temperature\r\n"
...
"  diag <scope>            scope: pd|rx|tx|txn|decoder|profile|unknown|\r\n"
  ...
  "\r\n"
  "  -- ML Packet Learning Engine (APIE_LEARN) --\r\n"
  "  learn on|off|dump|save|load|auto on|off|assign <idx> <name> <cmd>\r\n"
  "  signatures              list all learned packet signatures\r\n"
  "  learn export            print the signature table as CSV (also kept in flash)\r\n"
  "\r\n"
  "  -- Settings in the battery-backed CMOS --\r\n"
  "  cmos                    show the stored settings and where they come from\r\n"
  "  cmos save               write them to the backup SRAM now\r\n"
  "  cmos reset              factory defaults (same as removing the VBAT cell)\r\n"
  "  cmos faults             reset/fault breadcrumbs (why did it reboot?)\r\n"
  "  cmos clear              empty the fault log\r\n"
  "\r\n"
  "  -- External NOR flash store (replaces the SD card) --\r\n"
  "  store                   status: device, superblock, records, errors\r\n"
  "  store events [n]        show the last n events out of the flash\r\n"
  "  store selftest          non-destructive read/ID self test\r\n"
  "  store format            erase + rebuild the reserved 1 MB window\r\n"
  "  store on|off            allow / forbid writes (latched off on error)\r\n"
  "  store save|load         persist / restore the learned signatures\r\n"
  "  cmd list                owner command macros kept in the flash\r\n"
  "  cmd add <name> <cmd...> store one (runs through this same parser)\r\n"
  "  cmd show|del|run <name> inspect / delete / execute one\r\n"
  "\r\n"
  "  -- Failsafes --\r\n"
  "  usb [status|retry]      CDC console state (+ spend another attempt)\r\n"
  "  wdt                     watchdog + reset-cause status\r\n"
  "  wdt kick                feed the watchdog by hand\r\n"
  "  safe on|off             force / clear the degraded safe mode\r\n"
  "  sd status|mount|unmount SD card support (disabled at build time)\r\n"
  "\r\n"
  "  -- VDM Commands --\r\n"
  "  vdm identify            ask the source to identify itself (VDM Discover Identity)\r\n"
  "  vdm svids               list alternate-mode SVIDs the source supports\r\n"
  "  vdm modes <svid>        list the modes of an SVID (e.g. vdm modes ff01)\r\n"
  "\r\n"
...
static void handle_line(char *line)
{
  ...
  else if (strcmp(argv[0], "vdm") == 0)
  {
    if (argc < 2)
    {
      APP_LOG_Write("usage: vdm identify|svids|modes <svid>\r\n");
    }
    ...
    else if (strcmp(argv[1], "modes") == 0)
    {
      if (argc < 3)
      {
        APP_LOG_Write("usage: vdm modes <svid>   (e.g. vdm modes ff01)\r\n");
      }
      else
      {
        unsigned svid = 0;
        if (parse_h(argv[2], &svid) != 0)
        {
          APP_LOG_Write("usage: vdm modes <svid>   (e.g. vdm modes ff01)\r\n");
        }
        else
  ...
  else if (strcmp(argv[0], "console") == 0)
  {
    if (argc < 2)
    {
      ...
      APP_LOG_Printf("         cli input ring: %lu byte(s) dropped (paste burst overflow)\r\n",
                     (unsigned long)APP_CLI_RxDropped());
      APP_LOG_Printf("         usb tx watchdog resyncs: %lu (growth = wedged transfers)\r\n",
                     (unsigned long)APP_LOG_StuckResets());
  ...
  else if (strcmp(argv[0], "learn") == 0)
  {
    APIE_Learn_CliLearn(argc, argv);
  }
  else if (strcmp(argv[0], "signatures") == 0)
  {
    APIE_Learn_CliSignatures(argc, argv);
  }
  else if (strcmp(argv[0], "sdlog") == 0)
  {
    APIE_SdLog_Cli(argc, argv);
  }
  else if (strcmp(argv[0], "sd") == 0)
  {
    ...
    else if (strcmp(argv[1], "status") == 0)
    {
      APP_SD_Status(buf, sizeof(buf));
      APP_LOG_Printf("%s\r\n", buf);
      APP_SD_CardInfo(buf, sizeof(buf));
      APP_LOG_Printf("%s\r\n", buf);
    }
    else if (strcmp(argv[1], "mount") == 0)
    {
      if (APP_SD_TryMount())
      {
        APP_LOG_Write("sd: mounted\r\n");
      }
      else
      {
        APP_LOG_Write("sd: mount failed (no card?)\r\n");
      }
    }
    ...
    {
      APP_LOG_Write("usage: sd status|mount|unmount\r\n");
    }
  }
  else if (strcmp(argv[0], "cmos") == 0)
  {
    if ((argc >= 2) && (strcmp(argv[1], "save") == 0))
    {
      APP_CMOS_Save();
      APP_LOG_Write("cmos: settings written to the backup SRAM\r\n");
    }
    else if ((argc >= 2) && (strcmp(argv[1], "reset") == 0))
    {
      APP_CMOS_FactoryReset();
      APP_LOG_Write("cmos: factory defaults restored\r\n");
      APP_CMOS_Print();
    }
    else if ((argc >= 2) && (strcmp(argv[1], "faults") == 0))
    {
      APP_CMOS_PrintFaults();
    }
    else if ((argc >= 2) && (strcmp(argv[1], "clear") == 0))
    {
      APP_CMOS_ClearFaults();
      APP_LOG_Write("cmos: fault log cleared\r\n");
    }
    else
    {
      APP_CMOS_Print();
  ...
  else if (strcmp(argv[0], "store") == 0)
  {
    if (argc < 2)
    {
      APP_STORE_PrintStatus();
    }
    else if (strcmp(argv[1], "events") == 0)
    {
      uint32_t n = (argc >= 3) ? (uint32_t)strtoul(argv[2], NULL, 0) : 8U;
      if (n > 40U) { n = 40U; }
      APP_STORE_LogShow(n);
    }
    else if (strcmp(argv[1], "selftest") == 0)
    {
      /* Re-arm first: after a flash-mode fault the store is deliberately left
         alone at boot, and this is the command that walks back into it. */
      if (EXT_NOR_Ready() == 0U)
      {
        (void)EXT_NOR_Init();
      }
      APP_LOG_Printf("store: NOR self test %s\r\n", (EXT_NOR_SelfTest() != 0U) ? "PASS" : "FAIL");
      if (EXT_NOR_Ready() != 0U)
      {
        (void)APP_STORE_Init();
      }
    ...
    {
      APP_LOG_Write("store: erasing the reserved 1 MB window...\r\n");
      APP_LOG_Printf("store: %s\r\n",
                     (APP_STORE_Format() == 0) ? "formatted" :
                     "format failed (flash writes latched off?)");
    }
    else if (strcmp(argv[1], "on") == 0)
    {
      if (EXT_NOR_Ready() == 0U)
      ...
      EXT_NOR_SetEnabled(1U);
      if (APP_STORE_Ready() == 0U)
      {
        (void)APP_STORE_Init();
      }
      APP_LOG_Printf("store: writes enabled (%s)\r\n", EXT_NOR_Status());
    }
    else if (strcmp(argv[1], "off") == 0) { EXT_NOR_SetEnabled(0U); APP_LOG_Write("store: writes latched off\r\n"); }
    else if (strcmp(argv[1], "save") == 0) { (void)APIE_Learn_SaveStore(); }
    else if (strcmp(argv[1], "load") == 0) { (void)APIE_Learn_LoadStore(); }
    else { APP_STORE_PrintStatus(); }
  }
  else if (strcmp(argv[0], "cmd") == 0)
  {
    APP_CMD_Cli(argc, argv);
  }
  else if (strcmp(argv[0], "usb") == 0)
  {
    if ((argc >= 2) && (strcmp(argv[1], "retry") == 0))
    {
      USB_DEVICE_RetryNow();
    }
    USB_DEVICE_Status();
  }
  else if (strcmp(argv[0], "wdt") == 0)
  {
    APP_WDT_Print();
    if ((argc >= 2) && (strcmp(argv[1], "kick") == 0))
    {
      APP_WDT_Kick();
      APP_LOG_Write("wdt: kicked\r\n");
    }
  }
  else if (strcmp(argv[0], "safe") == 0)
  {
    if ((argc >= 2) && (strcmp(argv[1], "off") == 0))
    {
      APP_WDT_SetSafeMode(0U);
      APP_CMOS_ClearFaults();
      APP_LOG_Write("safe: cleared (reset the board to come back to the normal path)\r\n");
    }
    else if ((argc >= 2) && (strcmp(argv[1], "on") == 0))
    {
      APP_WDT_SetSafeMode(1U);
      APP_LOG_Write("safe: on (PD stays off from the next reset; 'safe off' clears it)\r\n");
    }
    else
    {
      APP_LOG_Printf("safe: %s\r\n", APP_WDT_InSafeMode() ? "ON" : "off");
    }
  }
  else if (apie_cli_dispatch(argc, argv) == 0)
  {
...
void APP_CLI_Execute(const char *line)
{
  ...
  /* Used by 'cmd run': feed a stored macro through the same parser the console
     uses.  app_cmd.c refuses nested execution, so the depth stays 1. */
