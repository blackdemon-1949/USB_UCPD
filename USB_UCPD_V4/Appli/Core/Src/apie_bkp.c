...
/* The first APP_CMOS_SIZE bytes of the 4 KB backup SRAM hold the boot
 * "CMOS" (settings + reset/fault log, app_cmos.c).  This backend owns the
 * remainder, so both live in VBAT-retained memory without a linker change:
 * BKPSRAM has a fixed architectural address and is never loaded or zeroed
 * by startup (it is addressed directly, not through a section). */
#include "app_cmos.h"
#define APIE_BKP_MEM ((volatile uint32_t *)(uintptr_t)(0x38800000UL + APP_CMOS_FREE_OFF))
#define APIE_BKP_MEM_WORDS ((4096u - APP_CMOS_FREE_OFF) / 4u)
_Static_assert(APP_CMOS_FREE_OFF < 4096u, "CMOS must leave room for apie_bkp");
...
/* Walk the section table of a validated payload.  Returns 1 and the four
...
static uint8_t bkp_parse_sections(const uint8_t *payload, uint32_t plen,
                                  ...
                                  const uint8_t **learn, uint32_t *learn_len)
{
  uint32_t off = 0u;
  ...
  *db   = NULL; *db_len   = 0u;
  *ml   = NULL; *ml_len   = 0u;
  *learn = NULL; *learn_len = 0u;

  if ((payload == NULL) || (plen < 8u))
  {
    return 0u;
  }
  ...
  *ml_len = get16(&payload[off]); off += 2u;
  if ((off + *ml_len) > plen) { return 0u; }
  *ml = &payload[off]; off += *ml_len;

  *learn_len = get16(&payload[off]); off += 2u;
  if ((off + *learn_len) != plen) { return 0u; }   /* no trailing garbage */
  *learn = &payload[off];
...
uint32_t APIE_Bkp_BuildBlob(uint8_t *buf, uint32_t size)
{
  ...
  APIE_BkpHeader_t h;
  uint8_t *payload;
  const APIE_LearnState_t *learn_state;
  ...
  /* Section 4: the learn engine state (auto-name, signatures, etc.) */
  learn_state = APIE_Learn_GetState();
  if (learn_state != NULL)
  {
    learn_len = (uint16_t)sizeof(APIE_LearnState_t);
    if ((off + 2u + learn_len) > (size - APIE_BKP_HDR_LEN))
    {
      return 0u;
    }
    put16(&payload[off], learn_len); off += 2u;
    memcpy(&payload[off], learn_state, learn_len);
    off += learn_len;
  }
  else
  {
    put16(&payload[off], 0u); off += 2u;
  }
...
uint8_t APIE_Bkp_LoadBlob(const uint8_t *buf, uint32_t size)
{
  APIE_BkpHeader_t h;
  const uint8_t *payload;
  const uint8_t *prof, *db, *ml, *learn;
  uint32_t prof_len, db_len, ml_len, learn_len;
  ...
  /* Import profiles first: on failure nothing has been touched yet.  All
     four importers validate their own content. */
  ...
  if ((learn_len != 0u) && (APIE_Learn_Import(learn, learn_len) == 0u))
  {
    return 0u;
  }
  s_seq = h.seq;
...
uint8_t APIE_Bkp_LoadProfiles(void)
{
  ...
  /* Explicit 'profile load': re-read the stored image and re-apply only the
     profile section (the DB/model/learn stay as they are). */
  if (size == 0u)
  {
    return 0u;
  }
