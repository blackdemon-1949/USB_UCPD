...
void APIE_OnCableAttach(uint8_t port, uint8_t cc)
{
  ...
  APIE_Learn_ResetSession(0, 0); /* VID/PID will be updated when known */
  s_unknown_last_ts = 0U;
  s_pend_attach = 1U;
  s_prev_volt_mv = 0.0f;
  if (s_safe_mode == 0U)
  ...
  /* Log attach event to SD */
  APIE_SdLog_Event("attach", s_conn_id);
}
...
void APIE_OnCableDetach(uint8_t port)
{
  ...
  /* Save learning data to SD */
  APIE_Learn_SaveToSD();
  APIE_SdLog_Event("detach", s_conn_id);
...
void APIE_OnDataInfo(uint8_t port, uint16_t data_id, const uint8_t *ptr, uint32_t size)
{
  (void)port;
  if (ptr == NULL) { return; }
  /* Feed learn response watcher before handling */
  if (size>0) APIE_Learn_OnResponse(0, ptr, (uint16_t)size);
  switch (data_id)
  {
...
void APIE_OnExtendedMessage(uint8_t port, uint16_t type, const uint8_t *data, uint16_t size)
{
  (void)port;
  if (data == NULL) { return; }
  if (size>0) APIE_Learn_OnResponse(0, data, size);
  switch (type)
  {
...
void APIE_OnVdmIdentity(uint8_t port, uint8_t ok)
{
  ...
  if (ok != 0U) {
    const APIE_Profile_t *pr = APIE_Profile_Get();
    if (pr && pr->has_hard) {
      APIE_Learn_ResetSession(pr->vid, pr->pid);
      APIE_Learn_UpdateSourceID(pr->vid, pr->pid);
    }
  }
}
...
void APIE_OnVdmSvids(uint8_t port, const uint16_t *svids, uint8_t n, uint8_t ok)
{
  ...
  if (ok != 0U && svids && n>0) {
    /* crude raw for learn reply: encode first SVID as payload */
    uint8_t tmp[4]; tmp[0]=svids[0]&0xFF; tmp[1]=(svids[0]>>8)&0xFF; tmp[2]=0; tmp[3]=0;
    APIE_Learn_OnResponse(0, tmp, 2);
  }
}
  ...
  for (i = 0U; i < n; i++)
  {
    ...
    /* Decode deterministically; if un-named, feed the unknown analyzer. */
    {
      ...
      if (cls == APIE_MSG_CLS_UNKNOWN)
      {
        ...
        /* Also feed the ML learning engine for ALL packets (not just unknown) */
        APIE_Learn_FeedPacket(pk->sop, pk->type, pk->ext, pk->nobjects,
                              pk->data, pk->len,
                              (uint32_t)s_last_voltage_mv, (uint32_t)s_last_current_ma,
                              (uint32_t)s_last_temp_c, s_pend_reset, s_pend_voltchg,
                              s_pend_attach);
        ...
        /* Log to SD */
        APIE_SdLog_Packet(pk->ts_ms, pk->dir, pk->sop, pk->msgid,
                          pk->type, pk->ext, pk->nobjects, pk->hdr,
                          pk->data, pk->len);
      }
      else
      {
        ...
        /* Also feed known packets to learning engine */
        APIE_Learn_FeedPacket(pk->sop, pk->type, pk->ext, pk->nobjects,
                              pk->data, pk->len,
                              (uint32_t)s_last_voltage_mv, (uint32_t)s_last_current_ma,
                              (uint32_t)s_last_temp_c, s_pend_reset, s_pend_voltchg,
...
static void record_outcome(uint8_t query, uint8_t success, uint32_t latency_ms)
{
  ...
  /* Heavy MLP: train on full 12-feature vector */
  {
    APIE_FeatureVec_t fv;
    build_current_features(&fv);
    APIE_Mlp_Observe(&fv, success);
  }
  APIE_Ml_Anomaly_Observe((float)latency_ms);   /* global */
  APIE_Ml_Anomaly_ObserveQ(query, (float)latency_ms); /* per-query */
  s_count_ml++;
  /* Log training row to SD (CSV) — failsafe: sdlog handles unmounted gracefully */
  {
    char row[224];
    int nn = snprintf(row, sizeof(row), "%lu,%u,%u,%u,%u,%u,%lu\r\n",
                      (unsigned long)HAL_GetTick(), (unsigned)query, 0u, (unsigned)has_pps, (unsigned)hard, (unsigned)success, (unsigned long)latency_ms);
    if (nn>0 && (size_t)nn < sizeof(row)) APIE_SdLog_TrainRow(row);
  }
...
void APIE_Task(void)
{
  ...
  /* Auto-query for newly learned signatures (confidence≥30%, count≥3).
     Runs when we have a contract and no pending query, under R1+. */
  if ((s_pending_query == 0xFFU) && (contract != 0U) && APIE_Exp_Allows(APIE_EXP_R1))
  {
    uint16_t learn_count = APIE_Learn_Count();
    for (uint16_t i = 0; i < learn_count; i++)
    {
      const APIE_LearnSig_t *sig = APIE_Learn_Get(i);
      if (sig != NULL && APIE_Learn_ShouldAutoQuery(sig))
      {
        if (APIE_Learn_ExecAutoQuery(i))
        {
          APP_LOG_Printf("[apie] auto-query for signature %u (%s)\r\n",
                         (unsigned)i, sig->cli_name[0] ? sig->cli_name : APIE_Learn_CatName(sig->category));
          break;  /* Only one auto-query per cycle */
        }
      }
    }
  }
  ...
  {
    feed_unknown();
    /* Heavy ML: run ensemble + per-Q anomaly scoring even when no packet arrived,
       to keep the engine “hot” and burn FLOPs as requested (bounded, <1ms). */
    if (s_count_txn > 0U) {
      APIE_FeatureVec_t fv_heavy;
      build_current_features(&fv_heavy);
      for (uint8_t _h=0; _h<3; _h++){
        volatile float _p = APIE_Mlp_Predict(&fv_heavy);
        (void)_p;
        uint8_t q = _h % APIE_NB_QUERY_BINS;
        volatile uint8_t _a = APIE_Ml_Anomaly_FlagQ(q, fv_heavy.v[0], 3.0f);
        (void)_a;
      }
    }
  }
