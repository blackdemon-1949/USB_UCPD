 ...
 *                 u16 learn_len
 *                 learn: APIE_LearnState_t (raw struct bytes)
 *
 * The payload covers everything the user or the engine ever customizes:
 * the PC13 owner profile list, the learned source profiles, the online
 * ML model, and the learn engine state.  The staging buffer is allocated
 * for the FULL 4 KiB window minus the header, so adding sections later
 * needs no re-sizing.
 *
 ...
 * / sizeof(APIE_LearnState_t) must bump the schema so an older image
 * is rejected instead of parsed.
 ******************************************************************************
 */
#ifndef APIE_BKP_H
#define APIE_BKP_H
...
#define APIE_BKP_SCHEMA      4u            /* 4 = learn engine state + MLP  */
#define APIE_BKP_HDR_LEN     32u           /* fixed header size, bytes     */
/* Structural payload minimum: four (u16 length + data) section headers,
 * all sections empty.  The section parser and the per-section importers do
 * the real validation. */
#define APIE_BKP_PAYLOAD_MIN 8u
