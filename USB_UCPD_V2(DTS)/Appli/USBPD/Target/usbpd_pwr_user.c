...
__weak int32_t BSP_USBPD_PWR_VBUSGetVoltage(uint32_t Instance, uint32_t *pVoltage)
{
  ...
  {
    /* CC-only tester: no VBUS ADC on the MCU header.  Report the synthetic
       value (last requested / 5 V while attached) so the PE can complete
       the explicit contract.  When an INA226 monitor is fitted on I2C2 and
       the user opted in with 'ina vbus real', report the measured rail
       instead (falls back to the synthetic value if the data is stale). */
    val = APP_PD_GetVbusMv();
    if ((INA226_VbusModeIsReal() != 0U) && (INA226_DataFresh() != 0U))
    {
      int32_t measured = INA226_GetBusMv();
      if (measured >= 0)
      {
...
__weak int32_t BSP_USBPD_PWR_VBUSIsOn(uint32_t Instance, uint8_t *pState)
{
  ...
  {
    uint32_t mv = APP_PD_GetVbusMv();
    /* With 'ina vbus real' the INA226 measurement wins while it is fresh. */
    if ((INA226_VbusModeIsReal() != 0U) && (INA226_DataFresh() != 0U))
    {
      mv = (uint32_t)INA226_GetBusMv();
    }
    state = (mv > USBPD_PWR_HIGH_VBUS_THRESHOLD) ? 1U : 0U;
    ret = BSP_ERROR_NONE;
  }
  *pState = state;
  return ret;
